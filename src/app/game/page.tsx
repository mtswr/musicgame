...
function GameContent() {
  ...
  const handleSkip = () => {
    // Evita múltiplos skips ou skip quando o modal já está visível
    if (showResultModal || isLoading || currentResult !== null) return;

    // Marca como carregando para evitar cliques múltiplos
    uiDispatch({ type: 'SET_LOADING', payload: true });
    ...
    if (attempts.length + 1 >= 6) {
      // Se chegou ao limite de tentativas, finaliza a faixa
      requestAnimationFrame(() => {
        finishTrack();
      });
    } else {
      // Caso contrário, avança para o próximo segmento
  ...
  const handleSubmit = async () => {
    // Evita múltiplas submissões ou submissão quando o modal já está visível
    if (showResultModal || isLoading || currentResult !== null) return;
    ...
    try {
      ...
      if (data.correct) {
        ...
        // Mostra o modal de resultado apenas após o processamento
        requestAnimationFrame(() => {
      ...
      // Em caso de erro, apenas avança para o próximo segmento
      const nextSegment = Math.min(attempts.length + 1, AUDIO_SEGMENTS.length - 1);
      gameDispatch({ type: 'SET_SEGMENT', payload: nextSegment });
    } finally {
      uiDispatch({ type: 'SET_LOADING', payload: false });
  ...
  const finishTrack = async () => {
    // Evita múltiplas chamadas e verifica se o modal já está aberto
    if (showResultModal || currentResult !== null) return;

    // Primeiro marca que estamos processando o resultado
    uiDispatch({ type: 'SET_LOADING', payload: true });
    ...
    try {
      ...
      uiDispatch({ type: 'SET_LOADING', payload: false });
      // Só abre o modal após o processamento completo
      uiDispatch({ type: 'TOGGLE_RESULT_MODAL', payload: true });
    }
  };
  ...
  const nextTrack = () => {
    // Certifique-se de que o áudio está pausado
    if (audioRef.current) {
      audioRef.current.pause();
      audioRef.current.currentTime = 0;
    }
    ...
    // Verifique se há mais faixas para jogar
    if (currentTrackIndex + 1 < tracks.length) {
      // Usar requestAnimationFrame para garantir que a UI foi atualizada antes da próxima ação
      requestAnimationFrame(() => {
        gameDispatch({ type: 'NEXT_TRACK' });
      });
    } else {
      // Não há mais faixas, mostre o resumo do jogo
      uiDispatch({ type: 'TOGGLE_RESULT_MODAL', payload: false });
      // Certifique-se de mostrar o resumo somente após a animação de fechamento
      requestAnimationFrame(() => {
  ...
  useEffect(() => {
    // Verifica se atingiu o limite de tentativas e se não está mostrando o modal ou já processando
    if (attempts.length >= 6 && !showResultModal && !isLoading && currentResult === null) {
      requestAnimationFrame(() => {
        finishTrack();
      });
    }
  }, [attempts.length, showResultModal, isLoading, currentResult]);
  ...
  return (
    <div className="grid grid-cols-2 gap-6 max-w-2xl mx-auto p-6">
      ...
      <div className="col-span-2 mt-4">
        <div className="relative">
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-zinc-900 border border-zinc-600 rounded-md shadow-lg z-10 max-h-60 overflow-y-auto">
              <div className="py-1">
      ...
      <GameResultDialog
        ...
        onOpenChange={(open: boolean) => uiDispatch({ type: 'TOGGLE_RESULT_MODAL', payload: open })}
      ...
      <GameSummaryDialog
        ...
        onOpenChange={(open: boolean) => uiDispatch({ type: 'TOGGLE_GAME_SUMMARY', payload: open })}
